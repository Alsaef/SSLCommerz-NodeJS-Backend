# SSLCommerz NodeJS Backend (TypeScript Version, MERN Style, Dynamic Params)

This is a full backend setup for integrating **SSLCommerz payment gateway** with a MERN stack using **TypeScript**. It handles payment initialization, success, fail, cancel, and optional IPN (Instant Payment Notification). The backend is fully MongoDB compatible and uses dynamic params for secure transaction tracking.

---

```bash
npm install sslcommerz-lts express cors mongodb
